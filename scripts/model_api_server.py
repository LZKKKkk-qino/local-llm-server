...
# 设置 CUDA 库路径，指向 PyTorch 自带的库文件夹
# 请确保这个路径下确实有 cublas64_xx.dll, cudnn64_xx.dll 等文件
os.environ['CUDA_PATH'] = r"D:\CUDA"
...
print('以下的 模型类class 是让构建的 API 完全兼容 OpenAI 的格式格式')
# 以下的 模型类class 是让构建的 API 完全兼容 OpenAI 格式
...
class ModelList(BaseModel):
    object: str = "list"
    data: List[ModelCard] = [ModelCard(id="glm-4"),
                             ModelCard(id="Qwen2.5-0.5B-Instruct"),
                             ModelCard(id="Qwen3-8B")]
...
def process_response(output: str, tools: dict | List[dict] = None, use_tool: bool = False) -> Union[str, dict]:
    ...
    arguments_json = None
    # 特殊工具集
    special_tools = ["cogview", "simple_browser"]  # 大模型中内置的函数
    # 外部指定的工具集
...
def process_messages(messages, tools=None, tool_choice="none"):
    ...
    if tool_choice != "none":  # 代表用户希望使用工具
        if isinstance(tool_choice, dict):
            tools = filter_tools(tool_choice, tools)
        if tools:  # 如果提供的工具列表中有可用符合的工具
            processed_messages.append(
                {
                    "role": "system",
                    "content": None,
                    "tools": tools
                } # 则把使用的工具信息放在 system 消息中
            )
            msg_has_sys = True
...
# 向模型输入请求参数，得到模型回复，并构建 ChatCompletionResponse 响应数据，符合OpenAI API 规范数据结构。
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    async for response in generate_stream(gen_params):
        pass  # 空循环，只是为了消耗生成器, 等待生成器结束得到最终结果
    ...
    # 循环遍历模型生成内容中里面的 usage 属性，像"prompt_tokens"，和里面属性的值
    for usage_key, usage_value in task_usage.model_dump().items():
        # 将 usage_value 添加到 usage_info 对象中对应属性去得到该次请求的 usage 信息
        # 后续传入完成响应 ChatCompletionResponse 的 usage 属性中去
...
async def predict_stream(model_id, gen_params):
    ...
    async for new_response in generate_stream(gen_params):
        ...
        if is_function_call:
            if not has_send_first_chunk:
                # 返回工具调用的第一个工具名称chunk
                # 工具调用指令也要将工具的内容（name，argument）分开返回 chunk
        ...
        # 如果满足这个条件，说明用户可能希望使用工具，但目前还不确定模型输出中有具体的工具调用，直到确定模型输出是否需要工具调用。
    ...
    # 兜底, 当上面的elif 用户想调用工具但模型最后都没有选择调用工具时
    # 会进入这个分支，并返回模型最终的回复
    elif delta_text != "":
        # 发出最后一个文本的chunk
        message = DeltaMessage(
...
if __name__ == "__main__":
    ...
    engine_args = AsyncEngineArgs(
        ...
        # dtype="bfloat16",  # 如果你的 GPU 支持 bf16
        dtype="float16",                # 如果不支持 bf16，用 fp16
        trust_remote_code=True,  # Qwen 必需
        gpu_memory_utilization=0.6,  # 0.5B 模型很小，可以用更高比例
        enforce_eager=True,  # Windows 下建议 True
        max_model_len=MAX_MODEL_LENGTH,
        enable_prefix_caching=False,  # 启用缓存
        max_num_seqs=32,  # 小模型可以处理更多并发
        block_size=16,
